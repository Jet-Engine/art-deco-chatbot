...
The Art-Deco era, spanning the roaring 1920s to the 1940s, the Art-Deco era left a dazzling legacy in architecture.  
...
- **main_model** : Specifies the main language model used in the project, which in this case is "llama3". 
- **embed_model** : Indicates the model used for generating embeddings, essential for the RAG functionality.
...
**This step is optional because the content files of all scraped articles are available in the `rag_files` 
directory, so there's no need to repeat the scraping process.**
...
There is a one quirky situation about `rag_files` directory though. If you run `wiki-bot.py` and start with
empty `rag_files` directory, the contents of the scraped articles would be saved into `text` sub-folder under
`rag_files`. The bot creates different sub-folders to store different types of data such as article urls, 
references etc. Since we only need contents of the Wikipedia articles for now to make less clutter after we run
the bot we copied the contents of `text` sub-folder to `rag_files` directory and get rid of everything else. 

Thus, if you would like to run the bot yourself which is not needed since you already have scraped documents
in `rag_files` directory, you need to copy all files of `text` sub-folder to `rag_files` directory and delete
all sub-folders under `rag_files` afterward or change `rag_files_path` in `config.yaml` to `rag_files/text`.
...
the `questions.csv` file, the files produced will be similar, although response times may vary if your 
computing power is different from ours.
...
embedding models and chunking techniques in future we skip a thorough evaluation of the quality of responses 
in this blogpost.

If you are interested you could compare results yourself by checking out generated tables that include 
responses and our document set that is indexed for RAG. Interestingly we could outsource this task to LLM's 
...
This project not only underscores the potential of RAG in enhancing the reliability of LLMs but also invites 
enthusiasts and developers to explore the intriguing domain of Art-Deco architecture through
