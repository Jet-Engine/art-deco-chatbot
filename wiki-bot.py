...
# Set up logging
logging.basicConfig(filename='wiki_bot_log.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
def get_listing_pages(url):
    try:
        ...
        for listing in listings:
            ...
            lists_url.append(urljoin(wikipedia_base_url, listing.attrs['href']))
...
def get_buildings_from_listing_pages(listing):
    ...
    for url, text in listing.items():
        ...
        navbar_links = set(link['href'] for link in soup.select(".navbox a[href]"))
        ...
        cat_links = set(link['href'] for link in soup.find_all('a', href=True) if link.find_previous(id='catlinks'))
        ...
        for li in lis:
            ...
            first_part_is_text = (len(text_parts) > 1 and isinstance(text_parts[0], str) and text_parts[0].strip())
    ...
    try:
        ...
        for element in content.find_all(content_tags):
            if element.name in header_tags + ['p']:
                ...
                src = element['src']
                full_image_url = 'https:' + src if src.startswith('//') else src
        ...
        if ref_section:
            for li in ref_section.find_all('li'):
                ...
                if link and 'href' in link.attrs:
                    data["references"].append({"text": reference, "url": link['href']})
        ...
        text_filepath = os.path.join(output_folder, directories['text'], base_filename + '.txt')
        url_filepath = os.path.join(output_folder, directories['url'], base_filename + '.url')
        imgs_filepath = os.path.join(output_folder, directories['imgs'], base_filename + '.imgs')
        refs_filepath = os.path.join(output_folder, directories['refs'], base_filename + '.json')
        html_filepath = os.path.join(output_folder, directories['html'], base_filename + '.html')
...
if __name__ == "__main__":
    ...
    for building in buildings:
        ...
        fetch_and_save_article(urljoin(wikipedia_base_url, building), download_path)
        pbar.update(1)
    pbar.close()
